...
public class AudioAnalysis : MonoBehaviour
{
    ...
    public static float[] samples = new float[512];        // 0 to 20,000 hz compacted into 128 samples
    public static float[] freqBand = new float[8];

    public static float[] bandBuffer = new float[8];
    public static float[] freqBandHighest = new float[8]; //keep track of highest values of each freqband to normalize
    float[] bufferDecrease = new float[8];

    float highest = 0;

    //band chosen to drive reactions
    public static int FocusBand;
